...
本 RFC 不设计前端页面，也不定义完整 HTTP API；它只保证后续 FastAPI、Next.js 和本地 Agent runtime 可以基于同一套领域语义工作。最重要的限制是：本期使用本地 SQLite 和单机状态版本，不做分布式并发控制，也不做真实地图服务。

## 动机

当前需求来自一个实际团队会务场景：成员需要查询和预订会议室，登录用户可以配置会议室、开放时段和不可预约规则。系统必须基于真实空间关系工作，尤其要正确处理以下固定约束，这些约束是领域不变量，普通配置不能覆盖或删除：

- 活动室在固定午餐时段作为餐厅使用，不能预约会议；
...
`Room` 表示真实物理空间，例如活动室、会议室一、会议室二、503、504、505、506。
...
| `type` | string | `activity`、`small`、`medium`、`large` 等业务类型；固定空间初始化中的 `activity-room`、`meeting-room-1`、`meeting-room-2`、`503`、`505`、`506` 的固定关系不可被普通配置覆盖 |
| `capacity` | int | 可容纳人数 |
| `location` | string | 楼层或区域，例如 `5F` |
| `equipment` | string[] | 设备，例如 `projector`、`whiteboard` |
| `position` | object | 可选，平面图坐标 `{x,y,width,height}`；默认房间必须提供，NL 新增房间可要求用户提供或由后端分配默认坐标 |
...
`RoomRule` 表示某个房间或组合空间在特定时间窗内不可预约、固定占用、维护、活动占用等状态。
...
| `rule_type` | string | `fixed_unavailable`、`weekly_unavailable`、`temporary_unavailable`、`maintenance`、`activity_block` 等 |
| `match_key` | string | 用于连续修改时匹配同一条规则 |
| `reason` | string | 展示原因，例如“临时维修” |
| `actor_id` | string | 执行创建或修改写操作的用户 ID；固定规则由系统初始化时写入，不能被普通配置覆盖 |
| `created_at` | datetime | 创建时间 |
| `updated_at` | datetime | 更新时间 |

规则可以有一个或多个 `RuleTimeWindow`。API 和 Agent Tool 统一使用 `time_windows: [{start_at, end_at, recurrence}]` 表达规则时间窗；自然语言配置如果连续修改同一个业务意图，应通过 `match_key` 或时间/目标相似度匹配到同一条规则并更新它。
...
| `target_id` | string | 目标房间或组合空间 ID |
| `title` | string | 会议标题 |
| `actor_id` | string | 执行创建或修改写操作的用户 ID |
| `organizer_id` | string | 会议发起人 |
...
| ID | 名称 | 类型 | 容量建议 | 设备建议 | 位置 | 平面图坐标 |
|---|---|---|---:|---|---|---|
| `activity-room` | 活动室 | `activity` | 20 | `projector`, `whiteboard` | 5F | `{x: 40, y: 40, width: 100, height: 60}` |
| `meeting-room-1` | 会议室一 | `medium` | 12 | `projector`, `whiteboard` | 5F | `{x: 160, y: 40, width: 100, height: 60}` |
| `meeting-room-2` | 会议室二 | `medium` | 12 | `projector`, `whiteboard` | 5F | `{x: 280, y: 40, width: 100, height: 60}` |
| `503` | 503 | `small` | 4 | `whiteboard` | 5F | `{x: 40, y: 120, width: 80, height: 50}` |
| `504` | 504 | `small` | 4 | `whiteboard` | 5F | `{x: 140, y: 120, width: 80, height: 50}` |
| `505` | 505 | `small` | 4 | `whiteboard` | 5F | `{x: 240, y: 120, width: 80, height: 50}` |
| `506` | 506 | `small` | 4 | `whiteboard` | 5F | `{x: 340, y: 120, width: 80, height: 50}` |
...
| 规则 | 目标 | 类型 | 时间 | 说明 |
|---|---|---|---|---|
| 活动室固定占用（午餐） | `activity-room` | `fixed_unavailable` | 每个工作日 12:00-13:00 | 该时段固定作为餐厅使用，不能预约会议 |
| 505 周二全天不可用 | `505` | `weekly_unavailable` | 每周二 00:00-24:00 | 固定不可用 |

> 设计决策：本期不引入“午餐”专用规则类型。活动室的午餐不可预约由普通 `fixed_unavailable` 规则表达，时间窗为每个工作日 12:00-13:00，展示原因写为“午餐占用”。这样它与维护、活动、临时禁用等其他不可预约状态共用同一套规则引擎和冲突校验路径。

固定规则是领域不变量的一部分：活动室午餐固定占用和 505 周二全天不可用不能被普通规则配置删除、覆盖或改为可预约。API 和 Agent Tool 对固定规则的修改/删除请求必须返回受保护规则错误。

#### 小会议室关系

503、505、506 的 `type` 必须为 `small`；本期也将 `504` 初始化为 `small` 以支撑动态禁用演示。小会议室关系参与“小会议室”查询，不能被前端硬编码替代。
...
本 RFC 定义领域层需要维护一个轻量状态版本 `state_revision`。每次成功写入房间、开放时段、规则、预约、取消或修改预约时递增。
...
所有会改变系统状态的写操作必须在一个数据库事务内完成：读取当前状态、匹配规则或预约、写入/更新领域对象、记录审计事件、记录幂等结果、递增 `state_revision`。事务失败时整体回滚，不允许出现规则已更新但审计未记录、预约已创建但日历不可见、状态版本已递增但数据未落库等部分生效状态。
...
| 查询房间详情 | `target_type: room`, `target_id` | 房间、开放时段、固定规则摘要 |
| 查询组合空间详情 | `target_type: composite`, `target_id` | 组合空间、成员房间、容量、设备 |
| 查询可用目标 | 时间窗、容量、设备、类型、是否允许组合 | 可用目标、不可用原因、冲突详情 |
| 校验冲突/可用性 | 目标、时间窗、容量、设备、类型 | `available`、`checks`、`conflicts`、`unavailable_reasons` |
| 查询日历时段 | 目标、日期或时间范围 | 时段列表、预约、规则、固定占用 |
| 查询预约列表 | 日期、目标、状态、分页 | 预约摘要列表、分页信息 |
| 查询预约详情 | `booking_id` | 预约、时间窗、目标、冲突历史、可取消/可修改状态 |
| 创建预约 | `target_type`、`target_id`、时间窗、会议信息、幂等键 | 预约结果或冲突原因 |
| 取消预约 | `booking_id`、原因 | 取消结果、释放时段 |
| 修改预约 | `booking_id`、新 `target_type`/`target_id`、新时间窗、调整原因 | 更新结果、被影响预约 |
| 创建或更新规则 | 目标、规则类型、`time_windows`、匹配键 | 规则结果、影响时段 |
| 创建或更新房间 | 房间基础信息、容量、设备、位置、平面图坐标 | 房间结果、影响视图 |
| 创建或更新开放时段 | 房间、weekday、start_time、end_time | 开放时段结果 |
| 查询规则列表 | 目标、日期范围、规则类型、是否包含固定规则、分页 | 规则摘要列表 |
| 查询平面图状态 | 楼层、时间 | 房间坐标、组合空间状态、状态、原因 |
| 执行 Agent 自然语言解析 | utterance、意图类型 | 结构化 parsed_changes 或候选目标 |
...
图读法：用户、Next.js 前端或本地 Agent runtime 不直接判断会议室是否可用，而是调用应用服务；应用服务统一委托规则引擎读取空间、规则、开放时段和预约状态，并返回可解释结果。
...
未采用。原因是固定占用、周二不可用、临时维修、组合会议室占用等场景都需要独立解释原因。如果只靠预约表，无法表达“为什么不可预约”，也无法支持连续修改同一条规则。
...
**T2: 初始化固定空间和规则**
- **范围**: 系统启动时幂等写入活动室、会议室一、会议室二、503、504、505、506、组合空间、活动室固定占用规则和 505 周二不可用规则。
- **验收标准**: 多次启动不会重复创建；默认空间与规则符合需求中的固定约束。

**T3: 实现规则引擎与时间冲突校验**
- **范围**: 实现开放时间校验、规则阻断校验、预约重叠校验和半开区间冲突算法。
- **验收标准**: 相邻预约不冲突，重叠预约冲突；固定占用和周二不可用规则能阻断预约并返回原因。

**T4: 实现组合会议室约束**
- **范围**: 实现组合空间查询、组合预约校验、成员房间被组合预约占用时的互斥规则。
- **验收标准**: 会议室一和会议室二合并预约后，二者不能再被分别预约；成员房间已有预约时组合预约被拒绝，并返回组合预约冲突。
...
- 时间窗重叠算法：重叠、相邻、包含、跨天。
- 活动室固定占用规则：使用明确工作日 `2026-08-04T12:00:00+08:00` 到 `2026-08-04T13:00:00+08:00` 验证午餐时段阻断，非午餐时段允许。
- 505 周二不可用：周二阻断，其他日期允许。
- 组合会议室约束：组合预约阻断成员房间分别预约；成员房间已有预约时组合预约被拒绝。
- 规则连续修改：同一 `match_key` 更新同一条规则，`time_windows` 只反映下午时段。
- 状态版本：每次成功写入后单调递增；失败事务不递增。
- 幂等键：同 key 同内容返回上一次结果，同 key 不同内容返回冲突。

### 集成测试

- 默认空间初始化包含 `activity-room`、`meeting-room-1`、`meeting-room-2`、`503`、`504`、`505`、`506` 和 `meeting-room-1-2`，不出现 `502`。
- 下周二 `2026-08-04T10:00:00+08:00` 到 `2026-08-04T11:00:00+08:00` 查询小会议室：503、506 可用，505 因 `WEEKLY_UNAVAILABLE` 不可用，504 可用。
- `2026-08-04T12:00:00+08:00` 到 `2026-08-04T13:00:00+08:00` 预约活动室被 `FIXED_UNAVAILABLE` 拒绝；`GET /api/calendar` 和 `GET /api/floor-plan` 同时间段也显示活动室固定占用。
- 本周五 `2026-08-07T14:00:00+08:00` 到 `2026-08-07T16:00:00+08:00` 创建会议室一+会议室二预约：成功后成员房间不可分别预约。
- 先创建会议室一预约，再创建会议室一+会议室二预约，应返回 `overlapping_composite_booking` 或 `BOOKING_CONFLICT`，日历和平面图显示成员房间被组合空间占用。
- 504 临时维修：先全天，再改为下午，只更新一条规则；`matched_rule_id` 相同，`state_revision` 只按成功写入递增；日历、平面图和可用查询都反映同一条下午规则。
- 取消预约：释放时段后日历和平面图不再显示 booked，`POST /api/availability:check` 和 `POST /api/bookings` 允许重新预约。
- 固定规则删除/修改返回 `PROTECTED_RULE` 或等价错误，活动室午餐和 505 周二不可用不能被普通配置覆盖。
- 自然语言新增/修改会议室、开放时段和规则后，`GET /api/rooms`、`GET /api/calendar`、`GET /api/floor-plan` 能读取到新状态。

### 手动验证

1. 启动服务并确认默认空间已初始化，包含 504 且不包含 502。
2. 在日历中查看 `2026-08-04` 活动室 12:00-13:00 固定占用时段，应显示固定占用。
...
无。当前领域边界、固定空间关系、规则模型、连续修改策略、固定规则保护和默认空间初始化范围已在需求澄清中确认。
