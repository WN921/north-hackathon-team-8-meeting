...
9. 用户进入日历视图，选择某个会议室和日期，查看预约、午餐占用、临时禁用和合并占用。
...
| `Slot` | 日历中的一个时间段，可能表示空闲、预约、午餐占用或禁用 |
...
1. **前端不直接判断规则**：所有冲突、午餐、临时禁用、组合空间约束都由 FastAPI 返回，前端只展示和触发操作。
...
- **验收标准**: 能展示默认会议室；日历能显示预约、午餐占用和规则占用。
